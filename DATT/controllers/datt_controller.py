...
class DATTController(Controller):
    def __init__(self, config: AllConfig, cntrl_config: DATTConfig):
        ...
        self.policy = self.algo_class.load(
            SAVED_POLICY_DIR / f"{self.datt_config.policy_name}", self.env
        )
        if self.datt_config.adaptive == True:
            edim = self.config.adapt_config.get_e_dim()
            self.adaptation_module = Adapation(
                self.datt_config.adaptation_type,
    ...
    def response(self, **response_inputs):
        ...
        if self.datt_config.adaptive:
            ...
            if self.datt_config.adaptation_type == "rma":
                wind_terms = self.adaptation_module.adaptation_step(
                    torch.tensor(self.history_rma).float()
                )
        ...
        if self.config.policy_config.ff_term is False:
            ...
            obs_ = np.hstack(
                [obs_, obs_[0:3] - rot.inv().apply(self.ref_func.pos(t))]
                + [
                    obs_[0:3] - rot.inv().apply(self.ref_func.pos(t + 3 * i * dt))
                    for i in range(self.config.policy_config.time_horizon)
                ]
            )
        ...
        self.history_rma = np.concatenate(
            (rma_adaptation_input[None, :, None], self.history_rma[:, :, :-1]), axis=2
        )
        ...
        self.history = np.concatenate(
            (adaptation_input[None, :, None], self.history[:, :, :-1]), axis=2
        )
