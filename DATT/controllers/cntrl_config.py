...
class MPPIConfig:
    ...
    sample_std = [
        ...
        0.7,
        0.7,
    ]  # standard deviation for sampling = [thrust (unit = hovering thrust), omega (unit = rad/s)]
    gamma_mean = 1.0  # learning rate
    gamma_Sigma = 0.0  # learning rate
    omega_gain = 40.0  # gain of the low-level controller
    discount = 0.99  # discount factor in MPPI
    a_min = [0.0, -5.0, -5.0, -2.0]  # bounds of sampling action = [thrust, omega (unit = rad/s)]
    a_max = [0.0, 5.0, 5.0, 2.0]
