...
class MPPIController(Controller):
    def __init__(self, config: AllConfig, cntrl_config: MPPIConfig):
        super().__init__()
    ...
    def response(self, **response_inputs):
        ...
        action = (
            self.mppi_controller.policy_with_ref_func(
                state=state_torch, time=t, new_ref_func=ref_func_obj
            )
            .cpu()
            .numpy()
        )
...
class MPPI_thrust_omega:
    ...
    def __init__(self, config: AllConfig, mppi_config: MPPIConfig):
        ...
        self.thrust_hover = self.config.drone_config.mass() * self.config.sim_config.g()

        # MPPI parameters
        self.t_H = np.arange(
            0, self.mppi_config.H * self.config.sim_config.dt(), self.config.sim_config.dt()
        )
    ...
    def sample(self):
        return torch.normal(
            mean=self.a_mean.repeat(self.mppi_config.N, 1, 1), std=self.sampling_std
        )
    ...
    def rollout_with_adaptation(self, startstate, actions, L1_adapt=torch.zeros(3)):
        ...
        self.u = self.u + self.config.sim_config.k() * (
            actions[:, :, THRUST] / self.config.drone_config.mass() - self.u
        )
        self.angvel = self.angvel + self.config.sim_config.k() * (
            actions[:, :, ANGVEL] - self.angvel
        )
        ...
        axisfactor = torch.where(
            angnorm < 1e-8, 0.5 + angnorm**2 / 48, torch.sin(hangnorm) / angnorm
        ).unsqueeze(2)
        ...
        self.timer.tic()
        accel = (
            u.unsqueeze(2) * z_from_q(states[:, :, QUAT])
            - self.config.sim_config.g() * e3.view(1, 1, 3)
            + L1_adapt[None, None, :]
        )
    ...
    def policy_with_ref_func(self, state, time, new_ref_func=None, L1_adapt=torch.zeros(3)):
        ...
        self.timer.tic()
        self.time_step = int(np.ceil(time / self.config.sim_config.dt()))
        ...
        state_ref = torch.as_tensor(
            new_ref_func.ref_vec(self.t_H + time).T, dtype=torch.float32
        ).unsqueeze(0)
        ...
        cost = self.mppi_config.alpha_p * torch.sum(
            torch.linalg.norm(poserr, dim=2), dim=1
        ) + self.mppi_config.alpha_R * torch.sum(
            qdistance_torch(states[:, :, QUAT], state_ref[:, :, QUAT]), dim=1
        )

        cost *= self.config.sim_config.dt()
        ...
        a_final[THRUST] = (
            a_final[THRUST] / self.config.drone_config.mass()
        )  # crazyflie takes care of the mass
        return a_final
