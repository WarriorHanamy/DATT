...
class TrajectoryEnv(BaseQuadsimEnv):
    ...
    def __init__(self, config: AllConfig, ref=None, seed=None, fixed_seed=False, **kwargs):
        ...
        self.action_space = spaces.Box(
            low=np.array([-9.8, -20, -20, -2]), high=np.array([30, 20, 20, 2])
        )
    ...
    def reset(self, state=None):
        ...
        if (self.reset_freq > 0 and self.reset_count % self.reset_freq == 0) or (
            self.reset_count > self.reset_thresh
        ):
            try:
                self.ref.reset()
        ...
        if self.body_frame:
            ...
            if self.fb_term:
                obs_ = np.hstack(
                    [obs_, fb]
                    + [
                        obs_[0:3] - rot.inv().apply(self.ref.pos(self.t + 3 * i * self.dt))
            ...
            if self.fb_term:
                obs_ = np.hstack(
                    [obs_, fb]
                    + [self.ref.pos(self.t + 3 * i * self.dt) for i in range(self.time_horizon)]
                )
            else:
                velquat = obs_[3:]
                obs_ = np.hstack(
    ...
    def reward(self, state, action):
        ...
        poscost = np.linalg.norm(
            state.pos - self.ref.pos(self.t)
        )  # min(np.linalg.norm(state.pos), 1.0)
