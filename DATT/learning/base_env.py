...
from config.configuration import EnvCondition
...
class BaseQuadsimEnv(Env):
    ...
    def reset(self, state: Optional[State_struct] = None):
        ...
        if state is None:
            ...
            state = State_struct(
                ...
                rot=R.from_euler(
                    "ZYX", init_config.sampler.sample_param(init_config.rot), degrees=True
                ),
        ...
        if wind_config.is_wind:
            self.dists = [ConstantForce(scale=self.wind_vector)]
        else:
            self.dists = []
    ...
    def step(self, action):
        ...
        if wind_config.is_wind and wind_config.random_walk:
            self.wind_vector += np.random.normal(
                loc=0, scale=wind_config.random_walk * self.dt, size=(3,)
            )
            self.dists = [ConstantForce(scale=self.wind_vector)]

        state = self.quadsim.step_angvel_raw(
            self.dt,
            u,
            angvel,
            dists=self.dists,
            linear_var=self.linear_var,
            angular_var=self.angular_var,
