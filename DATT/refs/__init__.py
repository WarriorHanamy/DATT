...
class TrajectoryRef(Enum):
    ...
    def ref(self, config: RefConfiguration, seed=None, env_diff_seed=False, **kwargs):
        if self._value_ == "gen_traj":
            return gen_trajectory.main_loop(
                saved_traj=config.ref_name, parent=Path().absolute() / "refs"
            )
        return {
            ...
            TrajectoryRef.SQUARE_REF: square_ref.SquareRef(
                altitude=0, D1=1.0, D2=0.5, T1=1.0, T2=0.5, **kwargs
            ),
            TrajectoryRef.CIRCLE_REF: circle_ref.CircleRef(
                altitude=0, rad=0.5, period=2.0, **kwargs
            ),
            TrajectoryRef.RANDOM_ZIGZAG: random_zigzag.RandomZigzag(
                max_D=np.array([1, config.y_max, config.z_max]),
                min_dt=0.6,
                max_dt=1.5,
                diff_axis=config.diff_axis,
                env_diff_seed=env_diff_seed,
                seed=seed,
                **kwargs,
            ),
            TrajectoryRef.RANDOM_ZIGZAG_YAW: random_zigzag_yaw.RandomZigzagYaw(
                max_D=np.array([1, config.y_max, config.z_max]),
                min_dt=0.6,
                max_dt=1.5,
                seed=seed,
            ...
            TrajectoryRef.POLY_REF: polynomial_ref.PolyRef(
                altitude=0,
                use_y=(config.y_max > 0),
                seed=seed,
                t_end=10.0,
                degree=7,
                env_diff_seed=env_diff_seed,
            ...
            TrajectoryRef.CHAINED_POLY_REF: chained_poly_ref.ChainedPolyRef(
                altitude=0,
                use_y=(config.y_max > 0),
                seed=seed,
                min_dt=1.5,
            ...
            TrajectoryRef.MIXED_REF: mixed_trajectory_ref.MixedTrajectoryRef(
                altitude=0,
            ...
            TrajectoryRef.POINTED_STAR: pointed_star.NPointedStar(
                random=True, seed=seed, env_diff_seed=env_diff_seed, **kwargs
            ),
            TrajectoryRef.CLOSED_POLY: closed_polygon.ClosedPoly(
                random=True, seed=seed, env_diff_seed=env_diff_seed, **kwargs
            ),
    ...
    def ref_cf(self, seed=None, env_diff_seed=False, **kwargs):
        ...
        return {
            ...
            TrajectoryRef.RANDOM_ZIGZAG_YAW: random_zigzag_yaw.RandomZigzagYaw(
                max_D=np.array([1, y_max, z_max]), min_dt=0.6, max_dt=1.5, seed=seed, **kwargs
            ),
            TrajectoryRef.SETPOINT: setpoint_ref.SetpointRef(setpoint=(0.5, 0.5, 0), **kwargs),
            TrajectoryRef.POLY_REF: polynomial_ref.PolyRef(
                altitude=0,
