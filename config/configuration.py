...
class WindConfiguration:
    ...
    dir: ConfigValue[np.ndarray] = ConfigValue[np.ndarray](
        default=np.array([0.0, 0.0, 0.0]), randomize=False
    )
...
class EnvCondition(Enum):
    ...
    def get_attribute(
        self, env
    ) -> Tuple[Any, int, Union[Tuple[float], float], Union[Tuple[float], float]]:
        """
...
class RefConfiguration:
    ...
    # Default trajectory reference when not specified on CLI
    default_ref: str = "random_zigzag"
